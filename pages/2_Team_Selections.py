...
group1, group2 = st.columns([0.5 , 0.5])
...
try:
    with group1:
        for i in range(1, total_groups+1):
            st.write(f"### Group {i}")
            st.dataframe(df[df['group_number'] == i][['golfer', 'pick_count','THRU','SCORE', 'TODAY','R1','R2','R3','R4','TOT']], 
                         hide_index=True, use_container_width=True, column_config=COLUMN_CONFIG_ERROR)
            if split_num == i:
                break
    ...
    with group1:
        for i in range(1, total_groups+1):
            if i != 1:
                st.write(f"### Group {i}")
            st.dataframe(df[df['group_number'] == i][['golfer', 'pick_count', 'TEE TIME']], 
                         hide_index=True, use_container_width=True, column_config=COLUMN_CONFIG_ERROR)
            if split_num == i:
                break
